...
async function checkServiceWorkerValidity(swUrl: string, config?: Config): Promise<boolean> {
  try {
...
export function register0(config?: Config) {
  if (
...
export function unregister0() {
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.ready
      ...
      .catch((error) => {
        console.error("Unable to unregister serviceWorker", error.message)
      })
  }
}
*/
