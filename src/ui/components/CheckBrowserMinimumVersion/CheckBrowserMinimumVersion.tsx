...
const CheckBrowserMinimumVersion = React.memo(({ children }: CheckBrowserMinimumVersionProps) => {
  const minimumVersions = useBrowserMinimumVersionZustand()
  
  const deviceFullVersion = useMemo(() => {
    return versionToNumArr(device.fullBrowserVersion)
